...
// Helpers
// -------
function typeCheck(name, grammarSource, tsSource) {
    ...
    const bundleDtsContents = generateTypes(grammars).replace(`from 'ohm-js'`, `from './hackyInMemoryOhmDecls'`);
...
`;
const createMainSource = (actionsSource) => dedent `
...
test('incorrect arity', t => {
    const diagnostics = typeCheck('g', grammarSources.g, createMainSource(`
      start(letters, x) {
        return letters.op();
      },
      letters(letterIter) {
...
test('extendSemantics + addOperation', t => {
    const source = createMainSource(defaultActionsSource) +
        dedent `
...
test('extendOperation', t => {
    const goodSource = createMainSource(defaultActionsSource) +
        dedent `
